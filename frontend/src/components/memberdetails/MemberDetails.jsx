...
function ViewMemberDetails() {
  ...
  const [allSelectedRolesTeams, setAllSelectedRolesTeams] = useState({roles: [], teams:[]});
  const [newRoleTeam, setNewRoleTeam] = useState({add: false, role:null, teams:null});
  ...
  const memberRoleArray = [
    {id: "1", role: "Volunteer", desc: "Limited access to areas of portal", value: "VOLUNTEER"},
    {id: "2", role: "Leader", desc: "Access to all areas excluding Director area", value: "LEADER"},
    { id: "3", role: "Director", desc: "Access to all areas of portal", value: "DIRECTOR"}
  ];
  ...
  const {date_joined, email, first_name, last_name,status, role_teams} = user;
    ...
    <ContainerWithNav>
      <div className={styles["view-member-wrapper"]}>
        ...
        <div className={styles["member-container"]}>
          <BackToMemberPortal />
          <div
            className={cx(styles["member-details-container"])}
          >
            {(errorOnLoading) && (
            ...
            {!Object.keys(user).length ? (
              ...
              <div className={cx("col-10", styles["view-member-fields-div"])}>
                ...
                <div className={styles["view-member-info-editable"]}>
                  <MemberStatus status={status} changeStatus={handleStatusChange}/>
                  {roles.map(
                    (role) => (
                      <section key={role}>
                        <MemberRole role={role} status={status} numRoles={roles.length} modifyRole={handleRoleChange}/>
                        <MemberTeams role={role} teams={role_teams[role]} status={status} modifyTeams={handleTeamChange} totalTeams={teamsArray} allSelectedTeams={allSelectedRolesTeams.teams} editing={editing} setEditing={setEditing}/>
                  ...
                  {status === 'ACTIVE' ? 
                    ...
                    (
                      <>
                        <AddNewRole selectedRolesForUser={allSelectedRolesTeams.roles} allRoles={memberRoleArray} addNewRole={handleAddNewRole}/>
                        <AssignEditTeams role={newRoleTeam.role} selectedTeamsForUser={allSelectedRolesTeams.teams} totalTeams={teamsArray} editRoleTeams={handleAddNewTeams} status={status} setTeamEditMode={cancelAddNewMember} />
                      </>
                    )
                    : Object.keys(user.role_teams).length < memberRoleArray.length && 
                      <p className={cx("mt-2",styles["add-team-role"], styles["view-member-info-editable"])} onClick={handleAddNewMember}>+ add role and team</p>
                    : null
                  }
                </div>
              </div>
